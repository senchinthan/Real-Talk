...
interface EditCodingBankPageProps {
  params: Promise<{
    id: string;
  }>;
}

const EditCodingBankPage = ({ params: paramsPromise }: EditCodingBankPageProps) => {
  ...
  useEffect(() => {
    const fetchBank = async () => {
      try {
        const response = await fetch(`/api/coding-banks/${id}`);
        if (response.ok) {
        ...
        <Card className="bg-card border-border">
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <Button asChild>
                <Link href="/admin/coding-banks">Back to Coding Banks</Link>
              </Button>
            </div>
          </CardContent>
        </Card>
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex items-center mb-6">
        ...
        <h1 className="text-2xl font-bold text-white">Edit Coding Question Bank</h1>
      </div>

      <Card className="bg-card border-border">
        <CardHeader>
          <CardTitle className="text-white">Bank Details</CardTitle>
          <CardDescription>Update the details of this coding question bank</CardDescription>
        </CardHeader>
        <CardContent>
        ...
        <CardFooter className="border-t border-border pt-4 flex justify-between">
          ...
          >
            <Link href={`/admin/coding-banks/${id}/questions`}>
              Manage Questions
            </Link>
          </Button>
          <Button
            ...
            className="border-border text-white hover:bg-muted"
          >
            <Link href={`/admin/coding-banks/${id}/view-questions`}>
              View All Questions
            </Link>
          </Button>
        </CardFooter>
...
export default EditCodingBankPage;

...
const CodingBanksPage = () => {
  ...
  useEffect(() => {
    const fetchBanks = async () => {
      try {
        const response = await fetch('/api/coding-banks');
        if (response.ok) {
          const data = await response.json();
          setBanks(data.data);
        } else {
          console.error('Failed to fetch coding question banks');
        }
      } catch (error) {
  ...
  const handleDelete = async (bankId: string) => {
    ...
    try {
      const response = await fetch(`/api/coding-banks/${bankId}`, {
        method: 'DELETE'
      });
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
          <div>
            <h1 className="text-3xl font-bold text-white">Coding Question Banks</h1>
            <p className="text-muted-foreground">Create and manage coding question banks</p>
          </div>
          <Button asChild className="bg-primary hover:bg-primary/90">
            <Link href="/admin/coding-banks/create" className="flex items-center gap-2">
            ...
            <CardContent className="flex flex-col items-center justify-center py-12">
              ...
              <p className="text-muted-foreground text-center max-w-md">
                {searchTerm || filterDifficulty !== 'all'
                  ...
                  : "You haven't created any coding question banks yet. Click 'Create Question Bank' to get started."}
              </p>
              {(searchTerm || filterDifficulty !== 'all') && (
                <Button 
                  variant="outline" 
                ...
                <CardContent className="p-6">
                  <div className="flex flex-col sm:flex-row justify-between gap-4">
                    ...
                    <div className="flex flex-wrap sm:flex-nowrap gap-2">
                      ...
                      >
                        <Link href={`/admin/coding-banks/${bank.id}/questions`}>
                      ...
                      <Button 
                        ...
                        className="border-border text-white hover:bg-muted"
                      >
                        <Link href={`/admin/coding-banks/${bank.id}/edit`}>
...
export default CodingBanksPage;

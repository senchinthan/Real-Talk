...
import { Plus, Trash2, Edit, Search, AlertCircle, PenTool } from 'lucide-react';
...
const TextBanksPage = () => {
  const [banks, setBanks] = useState<TextQuestionBank[]>([]);
    ...
    const fetchBanks = async () => {
      try {
        ...
        if (response.ok) {
          ...
          console.error('Failed to fetch text question banks');
          toast.error('Failed to load text question banks');
        }
      } catch (error) {
        console.error('Error fetching text question banks:', error);
        toast.error('Error loading text question banks');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleDelete = async (bankId: string) => {
    if (!confirm('Are you sure you want to delete this question bank? All questions in this bank will be inaccessible.')) {
      return;
    }

    try {
      const response = await fetch(`/api/text-banks/${bankId}`, {
        method: 'DELETE',
      });
      ...
      if (response.ok) {
        ...
        toast.error(error.error || 'Failed to delete question bank');
      }
    } catch (error) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
          <div>
            <h1 className="text-3xl font-bold text-white">Text Question Banks</h1>
            <p className="text-muted-foreground">Create and manage text interview question banks</p>
          </div>
          <Button asChild className="bg-primary hover:bg-primary/90">
            <Link href="/admin/text-banks/create" className="flex items-center gap-2">
            ...
            <CardContent className="flex flex-col items-center justify-center py-12">
              ...
              <p className="text-muted-foreground text-center max-w-md">
                {searchTerm || filterDifficulty !== 'all'
                  ...
                  : "You haven't created any text question banks yet. Click 'Create Question Bank' to get started."}
              </p>
              {(searchTerm || filterDifficulty !== 'all') && (
                <Button 
                  variant="outline" 
                ...
                <CardContent className="p-6">
                  <div className="flex flex-col sm:flex-row justify-between gap-4">
                    ...
                    <div className="flex flex-wrap sm:flex-nowrap gap-2">
                      ...
                      >
                        <Link href={`/admin/text-banks/${bank.id}/questions`}>
                      ...
                      <Button 
                        ...
                        className="border-border text-white hover:bg-muted"
                      >
                        <Link href={`/admin/text-banks/${bank.id}/edit`}>
